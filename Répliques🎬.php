...
$nbSlogans = $_SESSION['Repliques.txt_count'];

shuffle($tableauDonnees);

fctAfficheEntete("Quel film pour cette réplique ?");
...
foreach ($tableauDonnees as $ligne) {
    if ($ligne['No'] === 0) {
		...
		echo "<H2 style='font-style: italic;'>";
		echo $ligne['Slogan'];
...
//        echo "<DIV class='flex-bouton' id='sol".$i."' onclick=\"clicVerif(this,'".$tableauDonnees[$i]['No']."');\">";
            ...
            echo "<IMG width=100% src='".fctBingSrc("film poster ".str_replace("'"," ",$tableauDonnees[$i]['Marque']),600)."' />";
            echo "</DIV>";
...
echo "<BR>$nbSlogans répliques en stock";
