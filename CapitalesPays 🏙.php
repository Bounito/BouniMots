...
foreach ($lignesAuHasard as $ligne) {
    ...
    $tableauDonnees[] = ['Marque' => trim($parties[1]), 'Slogan' => trim($parties[0]), 'No' => $i];
    $i++;
}
...
fctAfficheEntete("Quelle Capitale pour ce Pays ?");
...
foreach ($tableauDonnees as $ligne) {
    if ($ligne['No'] === 0) {
		...
		echo "<H2>";
        fctAfficheImage("Flag ".$ligne['Slogan'],100);
		echo "&nbsp;".$ligne['Slogan'];
...
for ($i=0;$i<$nbMots;$i++) {
    echo "<DIV class='flex-3x3'>";
        echo "\n<DIV class='flex-bouton' id='sol".$i."' onclick=\"clicDivBouton(this,'$motChoisi','".$tableauDonnees[$i]['Marque']."','monFormulaire',0,8,".$_SESSION['WinStreak'].");\">";
            ...
            echo "<IMG width=100% src='".fctBingSrc("City ".str_replace("'","",$tableauDonnees[$i]['Marque']),600)."' />";
            echo "</DIV>";
